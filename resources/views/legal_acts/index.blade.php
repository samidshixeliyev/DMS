            ...
            <div class="modal-content">
                <form action="{{ route('legal-acts.store') }}" method="POST">@csrf
                    <div class="modal-header">
                        <h5 class="modal-title"><i class="bi bi-plus-circle me-2"></i>Yeni sənəd</h5><button type="button"
                            class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        <div class="row g-3">
                            <div class="col-md-6"><label class="form-label">Sənədin nömrəsi <span
                                        class="text-danger">*</span></label><input type="text" name="legal_act_number"
                                    class="form-control" value="{{ old('legal_act_number') }}" required></div>
                            <div class="col-md-6"><label class="form-label">Sənədin tarixi <span
                                        class="text-danger">*</span></label><input type="text" name="legal_act_date"
                                    class="form-control modal-datepicker" value="{{ old('legal_act_date') }}" required>
                            </div>
                            <div class="col-md-6"><label class="form-label">Növü <span
                                        class="text-danger">*</span></label><select name="act_type_id"
                                    class="form-select modal-select2" required>
                                    <option value="">Seç</option>@foreach($actTypes as $t)<option value="{{ $t->id }}" {{ old('act_type_id') == $t->id ? 'selected' : '' }}>{{ $t->name }}</option>@endforeach
                                </select></div>
                            <div class="col-md-6"><label class="form-label">Kim qəbul edib <span
                                        class="text-danger">*</span></label><select name="issued_by_id"
                                    class="form-select modal-select2" required>
                                    <option value="">Seç</option>@foreach($issuingAuthorities as $a)<option
                                        value="{{ $a->id }}" {{ old('issued_by_id') == $a->id ? 'selected' : '' }}>
                                        {{ $a->name }}
                                    </option>@endforeach
                                </select></div>
                            <div class="col-md-6"><label class="form-label">Əsas icraçı <span
                                        class="text-danger">*</span></label><select name="main_executor_id"
                                    class="form-select modal-select2" required>
                                    <option value="">Seç</option>@foreach($executors as $e)<option value="{{ $e->id }}" {{ old('main_executor_id') == $e->id ? 'selected' : '' }}>
                                        {{ $e->name }}{{ $e->department ? ' — ' . $e->department->name : '' }}
                                    </option>@endforeach
                                </select></div>
                            <div class="col-md-6"><label class="form-label">Digər icraçı</label><select
                                    name="helper_executor_id" class="form-select modal-select2">
                                    <option value="">Seç</option>@foreach($executors as $e)<option value="{{ $e->id }}" {{ old('helper_executor_id') == $e->id ? 'selected' : '' }}>
                                        {{ $e->name }}{{ $e->department ? ' — ' . $e->department->name : '' }}
                                    </option>@endforeach
                                </select></div>
                            <div class="col-md-6"><label class="form-label">İcra müddəti</label><input type="text"
                                    name="execution_deadline" class="form-control modal-datepicker"
                                    value="{{ old('execution_deadline') }}"></div>
                            <div class="col-md-6"><label class="form-label">Tapşırıq №</label><input type="text"
                                    name="task_number" class="form-control" value="{{ old('task_number') }}"></div>
                            <div class="col-12"><label class="form-label">Qısa məzmun <span
                                        class="text-danger">*</span></label><textarea name="summary" class="form-control"
                                    rows="3" required>{{ old('summary') }}</textarea></div>
                            <div class="col-12"><label class="form-label">Tapşırıq</label><textarea name="task_description"
                                    class="form-control" rows="2">{{ old('task_description') }}</textarea></div>
                            <div class="col-md-6"><label class="form-label">Əlaqəli sənəd №</label><input type="text"
                                    name="related_document_number" class="form-control"
                                    value="{{ old('related_document_number') }}"></div>
                            <div class="col-md-6"><label class="form-label">Əlaqəli sənəd tarixi</label><input type="text"
                                    name="related_document_date" class="form-control modal-datepicker"
                                    value="{{ old('related_document_date') }}"></div>
                        </div>
                    </div>
                    <div class="modal-footer"><button type="button" class="btn btn-secondary"
                            data-bs-dismiss="modal">İmtina</button><button type="submit" class="btn btn-primary"><i
                                class="bi bi-save me-1"></i> Yarat</button></div>
                </form>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form id="editForm" method="POST">@csrf @method('PUT')
                    <div class="modal-header">
                        <h5 class="modal-title"><i class="bi bi-pencil me-2"></i>Redaktə</h5><button type="button"
                            class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    ...
                    <div class="modal-footer"><button type="button" class="btn btn-secondary"
                            data-bs-dismiss="modal">İmtina</button><button type="submit" class="btn btn-primary"><i
                                class="bi bi-save me-1"></i> Yenilə</button></div>
                </form>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        async function showDetails(id) {
            ...
            if (data.status_logs && data.status_logs.length > 0) {
                logsHtml = '<div class="timeline">'; data.status_logs.forEach(function (log) {
